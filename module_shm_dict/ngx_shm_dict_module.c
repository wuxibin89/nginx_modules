...
static char*  ngx_http__shm_zone(ngx_conf_t *cf,ngx_command_t *cmd, void *conf);
...
static ngx_int_t ngx_shm_dict_list_init(ngx_conf_t *cf) {
    ...
    if (g_shm_dict_list == NULL) {
        ...
        if(ngx_array_init(g_shm_dict_list,cf->pool, 1,
    ...
    for (i = 0; i < g_shm_dict_list->nelts; i++) {
        ...
        if ( ngx_strcmp(name->data,zone_t[i]->shm.name.data) == 0 ) {
            ...
            ngx_conf_log_error(NGX_LOG_EMERG, cf, 0,
                    "[_shm_zone] shm_dict_name repeat loading %s ",name);
            return NGX_ERROR;
        }
    }
    return NGX_OK;
...
static ngx_int_t ngx_shm_dict_list_push(ngx_conf_t *cf,ngx_str_t *name,
    ...
    if(zone == NULL){
        ...
        ngx_conf_log_error(NGX_LOG_EMERG, cf, 0,
                "[_shm_zone] ngx_pcalloc ngx_shm_dict_init is error");
        return NGX_ERROR;
    }
    ...
    *zp = zone;
    ...
    ngx_conf_log_error(NGX_LOG_INFO, cf, 0,
            "[_shm_zone] g_shm_dict_list push %s is ok",zone->shm.name.data);
...
static char * 
ngx_http__shm_zone(ngx_conf_t *cf, ngx_command_t *cmd, void *conf){
    ssize_t                    size;
    ...
    for (i = 1; i < cf->args->nelts; i++) {
        ...
        if (ngx_strncmp(value[i].data, "max_size=", 9) == 0) {
            ...
            if (size == NGX_ERROR) {
                ...
                ngx_conf_log_error(NGX_LOG_EMERG, cf, 0,
                        "[_shm_zone] invalid zone size \"%V\"", &value[i]);
                return NGX_CONF_ERROR;
            }

            if (size < (ssize_t) (200 * ngx_pagesize)) {

                ngx_conf_log_error(NGX_LOG_EMERG, cf, 0,
                        "[_shm_zone] zone \"%V\" is too small", &value[i]);
                return NGX_CONF_ERROR;
            }
        ...
        ngx_conf_log_error(NGX_LOG_EMERG, cf, 0,
                "[_shm_zone] invalid parameter \"%V\"", &value[i]);
        return NGX_CONF_ERROR;
    }
    ...
    if (name.len == 0) {
        ...
        ngx_conf_log_error(NGX_LOG_EMERG, cf, 0,
                "[_shm_zone] \"%V\" must have \"zone\" parameter",
                &cmd->name);
        return NGX_CONF_ERROR;
    }
